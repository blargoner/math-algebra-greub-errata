...
\item (37, 6): in problem~9, condition (ii) should be \(H_2\supset E_2+F_2\).
...
\section*{Chapter~IV}
\begin{itemize}
\item (103, 15): in~(4.6), it should be noted that \(\widehat{x}_j\)~indicates that the vector~\(x_j\) is deleted.\footnote{This notation is defined later in a footnote on p.~198.}
...
Errata are currently only listed for Chapters 1, 2 (except \S~10--17), and 3--9. As above, minor purely typographical inconsistencies are not listed.
...
\item (121, -6): the notation~\(x\dsum y\) has not been defined.
\item (123, -3): in the displayed equation, \(k\in E\) should be \(k\in F\).
\item (123, -1): in~(5.50), the notation~\(h\dsum k\) has not been defined.
...
\item (172, 5): in the displayed equation, the notation~\(\dsum\) has not been defined for vectors.
...
\section*{Chapter~8}
\begin{itemize}
\item (193, -15): ``see Section~1.25'' should be ``(see Section~1.25)''.
\item (193, -13): it should be noted that \(E^*\)~is dual to~\(E\).
\item (195, 13): it should be noted that~\(L_E\) is \(L(E;E)\).
\item (195, -7): ``(see Section~(6.2)'' should be ``(see Section~6.2)''.
\item (196, 13): in this section, the notation~\(\dsum\) has not been defined for vectors.
\item (197, -5): for the second projection, ``\(\after\)'' should be ``\(:\)''.
\item (198, -11): in~(8.8), \(\Psi_E^{-1}\) should be \(\Psi_E\) (twice).
\item (199, 4): in the displayed equation, \(t\)~should be~\(\unit\).
\item (199, 7): in~(8.9), \(t\)~should be~\(\unit\) (twice).
\item (199, -7), (199, -5): in (8.12) and~(8.13), the superscripts \(\hat{\imath}\) and~\(\hat{\jmath}\) should be \(i\) and~\(j\), respectively.
\item (199, -3): in the displayed equation, the superscripts \(\hat{\imath}\) and~\(\hat{\jmath}\) should be \(i\) and~\(j\), respectively, and \(\bigeprod^{2j}E^*\) should be \(\bigeprod^{2i}E^*\).
\item (200, 5): in the displayed equation, the superscript~\(\hat{p}\) should be~\(p\) (twice).
\item (200, 8): ``Section~6.18'' should be ``Section~6.17''.
\item (200, 10): ``(6.33)'' should be ``(6.34)''.
\item (200, -3): ``the \emph{Pfaffian} of \(\varphi_1,\ldots,\varphi_m\)'' should be ``the \emph{Pfaffian} of \(\varphi_1,\ldots,\varphi_m\) with respect to~\(a\)''.
\item (202, 11): in the displayed equation, the notation~\(\dsum\) has not been defined for vectors.
\item (203, 3): in Proposition~8.5.1, in the displayed equation, \(\Pf_b(\varphi)\)~should be~\(\Pf_b(\psi)\).
\item (203, -8): ``\(\Pf_E\)~is changed to~\(-\Pf_E\)'' should be ``the sign of~\(\Pf_E\) is changed''.
\item (203, -3): in the displayed equation, \(E\dsum F\) should be \(E\dsum E\) and it should be noted that the orientation of \(E\dsum E\) is induced from that of~\(E\).
\item (203, -2): in Example~1, it should be assumed that the orthonormal basis is positively oriented.
\item (204, -10): in this section, the notation~\(\dsum\) has not been defined for vectors.
\item (208, 6): in the displayed equation, the superscript~\(\hat{p}\) should be~\(p\) (twice).
\end{itemize}
